...
public class AlgoSPAM{
	 ...
	 * @param requestedPrefix 
	 * @throws IOException exception if error while writing the file or reading
	 */
	public void runAlgorithm(String input, String outputFilePath, double minsupRel, int requestedPrefix, AlgorithmHandler algorithmHandler) throws IOException {
		...
		// RUN THE ALGORITHM
		spam(input, minsupRel, requestedPrefix, algorithmHandler); 
		// record end time
	 ...
	 * @throws IOException 
	 */
	private void spam(String input, double minsupRel, int requestedPrefix, AlgorithmHandler algorithmHandler) throws IOException{
...
//						determineSection(bitindex - previousBitIndex);  // register the sequence length for the bitmap
		...
		// STEP2.1: OUTPUT ALL PATTERNS OF SIZE 1
		//System.out.println("STEP 2.1 : output all size 1");
		List<String> itemsSIDs = new ArrayList<>();
		for (Integer i : frequentItems) {
			itemsSIDs.add(verticalDB.get(i).getSIDs(sequencesSize));
		}
		outputItems(frequentItems, itemsSIDs, algorithmHandler);
		...
		for(Entry<Integer, Bitmap> entry: verticalDB.entrySet()){
			if (requestedPrefix < 0 || entry.getKey() == requestedPrefix) {
				...
				// items.
				dfsPruning(prefix, entry.getValue(), frequentItems, frequentItems, entry.getKey(), 2, algorithmHandler);
			}
		}
	}
	 ...
	 * @param frequentItems The list of items to output
	...
	private void outputPattern(Prefix prefix, String sIDs, AlgorithmHandler algorithmHandler) {
		...
		for(Itemset itemset : prefix.getItemsets()){
			...
			if (itemset.size() != 1) {
				if (itemset.size() == 0) {
					System.out.println("WARNING !! Itemset in pattern with size 0");
				} else {
					System.out.println("WARNING !! Itemset in pattern with size > 1 : "+itemset);
				}
			} else {
	 ...
	 */
	private void dfsPruning(Prefix prefix, Bitmap prefixBitmap, List<Integer> sn, List<Integer> in, int hasToBeGreaterThanForIStep, int m, AlgorithmHandler algorithmHandler) throws IOException {
//		System.out.println(prefix.toString());
			 ...
			 if(newBitmap.getSupport() >= minsup) {
				// save the pattern to the file
				 if(m >= minimumPatternLength) {
		            	savePattern(prefixSStep, newBitmap, algorithmHandler);
		        }
				// recursively try to extend that pattern
				if(maximumPatternLength > m ){
					dfsPruning(prefixSStep, newBitmap, sTemp, sTemp, item, m+1, algorithmHandler);
				}
			}
		...
		for(int k=0; k < iTemp.size(); k++){
			...
			// save the pattern
			 if(m >= minimumPatternLength) {
	            	savePattern(prefixIStep, newBitmap, algorithmHandler);
	        }
			// recursively try to extend that pattern
			if(maximumPatternLength > m){
				dfsPruning(prefixIStep, newBitmap, sTemp, iTemp, item, m+1, algorithmHandler);
			}
		}	
	 ...
	 * @throws IOException exception if error while writing to the file
	 */
	private void savePattern(Prefix prefix, Bitmap bitmap, AlgorithmHandler algorithmHandler) throws IOException {
		patternCount++;
...
//		System.out.println(r.toString());
		...
		// Sends the pattern to the clients
		String sIDs = bitmap.getSIDs(sequencesSize);
		outputPattern(prefix, sIDs, algorithmHandler);
	}
